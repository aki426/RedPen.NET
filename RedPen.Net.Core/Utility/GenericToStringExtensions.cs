...
namespace RedPen.Net.Core.Utility
{
    public static class GenericToStringExtensions
    {
        ...
        public static string ToStringExt<K, V>(this List<Dictionary<K, V>> listOfDics) =>
            "[" + string.Join(", ", listOfDics.Select(kvp => kvp.ToStringExt())) + "]";

        public static string ToStringExt<T>(this List<List<List<T>>> list3) =>
            "[" + string.Join(", ", list3.Select(l => l.ToStringExt())) + "]";
